...
{if $USE_GROUP_TABS}
    ...
    {foreach from=$moduleTopMenu item=module key=name name=moduleList}
        {if $name == $MODULE_TAB}
            <span class="dropdown-toggle">{sugar_link id="moduleTab_$name" module=$name data=$module}</span>
            <ul class="dropdown-menu">
                {if count($shortcutTopMenu.$name) > 0}
                    <h5>{$APP.LBL_LINK_ACTIONS}</h5>
                ...
                {if $name == 'Home' and !$lock_homepage}
                   <h5>{$APP.LBL_LINK_ACTIONS}</h5>
                    <li style="margin-top:5px; margin-bottom:5px;"><a href=""
                                                                      onclick="addDashboardForm();return false">{$APP.LBL_ADD_TAB}</a>
                    </li>
                    <li style="margin-top:5px; margin-bottom:5px;"><a href=""
                                                                      onclick="return SUGAR.mySugar.showDashletsDialog();">{$APP.LBL_ADD_DASHLETS}</a>
                    </li>
                {/if}
             <h5 class="home_h2">{$APP.LBL_LAST_VIEWED}</h5><br>
    ...
    <a title="{$item.module_name}"
       ...
       href="{sugar_link module=$item.module_name action='DetailView' record=$item.item_id link_only=1}">
        <span>{$item.item_summary_short}</span>
                                ...
                                <em><a href="{sugar_link module=$item.module_name action='EditView' record=$item.item_id link_only=1}"
                                       style="margin-left:10px;"><img style="float:right;"
                                                                      src="index.php?entryPoint=getImage&imageName=dashlet-header-edit.png"
                                                                      width="14" height="14"
                                                                      class="iconed_dull"></a></em>
                            </span>
                            </li>
...
{else}
        ...
        {if $name == $MODULE_TAB}
            ...
            {if count($shortcutTopMenu.$name) > 0}
               <h5>{$APP.LBL_LINK_ACTIONS}</h5>
                ...
                <br>
            {/if}
         <h5 class="home_h2">{$APP.LBL_LAST_VIEWED}</h5>
            <br>
            {foreach from=$recentRecords item=item name=lastViewed}
                {if $name == 'Home'}
    ...
    {if count($moduleExtraMenu) > 0}
        <li class="dropdown">
            <a href="#" class="dropdown-toggle" data-toggle="dropdown">{$APP.LBL_MORE} &raquo;</a><br/>
            <ul class="dropdown-menu">
                {foreach from=$moduleExtraMenu item=module key=name name=moduleList}
                    <li class="dropdown">{sugar_link id="moduleTab_$name" module=$name data=$module}</li>
                {/foreach}
                </ul>
        </li>
    {/if}
